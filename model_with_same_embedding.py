...
class Translator(object):
    ...
    def model(self):
        ...
        zh_embedding_matrix = tf.get_variable(name='zh_embedding_matrix',
                                              ...
                                              # regularizer=tf.nn.l2_loss,
                                              initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        conception_matrix = tf.get_variable(name='conception_matrix',
                                            shape=(FLAGS.embedded_size,FLAGS.conception_size),
                                            dtype= tf.float32,
                                            initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        ...
        # same embedding used here:
        # the word embedding come from the same embedding_matrix called conception_matrix
        with tf.device('/cpu:0'):
            en = self._concept(inputs,en_embedding_matrix,conception_matrix)
            zh = self._concept(targets,zh_embedding_matrix,conception_matrix)
        ...
        with tf.name_scope("decoder"):
            ...
            if FLAGS.is_inference:
                zh_concept = lambda ids:self._concept(ids,zh_embedding_matrix,conception_matrix)
                helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(zh_concept, start_tokens, end_token)
            else:
                helper = tf.contrib.seq2seq.TrainingHelper(zh, zh_len_sequence)
    ...
    def _concept(self,ids,embedding_matrix,conception_matrix):
        return tf.matmul(tf.nn.softmax(tf.nn.embedding_lookup(embedding_matrix,ids)),conception_matrix)
